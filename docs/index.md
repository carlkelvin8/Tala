# Kalasag-Tala National Service Training Program Management System — Documentation

Welcome to the official documentation for **Kalasag-Tala**, a comprehensive management system for the National Service Training Program (NSTP). This documentation covers everything from local setup to full API reference.

---

## Table of Contents

- [Overview](./overview.md)
- [Getting Started](./getting-started.md)
- [Architecture](./architecture.md)
- [Database Schema](./database.md)
- [API Reference](./api-reference.md)
- [Frontend Guide](./frontend.md)
- [Authentication & Roles](./auth.md)
- [Features](./features.md)
- [Deployment](./deployment.md)
- [Environment Variables](./environment.md)

---

## Quick Links

| Topic | Description |
|---|---|
| [Getting Started](./getting-started.md) | Set up the project locally |
| [API Reference](./api-reference.md) | Full REST API documentation |
| [Auth & Roles](./auth.md) | Authentication flow and role permissions |
| [Database Schema](./database.md) | All models and relationships |
| [Deployment](./deployment.md) | Deploy to Railway and Netlify |
