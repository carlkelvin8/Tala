...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl border border-silver/20 bg-white px-3.5 py-2.5 shadow-lg">
        <p className="text-[11px] font-medium text-darksilver">{label}</p>
        <p className="mt-0.5 text-sm font-bold text-black">
          {payload[0].value}{" "}
          <span className="text-xs font-normal text-darksilver">present</span>
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-2xl border border-silver/20 bg-white shadow-card">
      ...
      <div className="flex flex-col gap-3 border-b border-silver/20 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-1 self-start rounded-xl border border-silver/20 bg-white p-1 sm:self-auto">
        ...
        {isLoading ? (
          ...
          <div className="flex h-56 flex-col items-center justify-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white">
              <svg className="h-5 w-5 text-silver" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
              </svg>
            </div>
            <p className="text-xs text-darksilver">No attendance data for this period</p>
          </div>
        ) : (
          <div className="h-56">
