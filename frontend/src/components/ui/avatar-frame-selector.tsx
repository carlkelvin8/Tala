...
export function AvatarFrameSelector({ user, selectedFrame, onSelectFrame }: AvatarFrameSelectorProps) {
  ...
  return (
    <div className="space-y-4"> {/* Vertical stack with spacing between the header, grid, and description */}
      <div> {/* Header section */}
        <h4 className="text-sm font-semibold text-black mb-1">Avatar Frame Style</h4> {/* Section title */}
        <p className="text-xs text-darksilver">Choose how your profile picture appears</p> {/* Subtitle explaining the purpose */}
      </div>
        ...
        {frames.map((frame) => { // Iterate over each available frame type
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex flex-col items-center gap-2 rounded-lg border-2 p-2 transition-all hover:border-silver/50", // Base styles: relative positioning, flex column, centered, rounded, border, padding, hover effect
                isSelected
                  ? "border-black bg-white shadow-sm" // Selected state: dark border, light background, subtle shadow
                  : "border-silver/30 bg-white" // Unselected state: light border, white background
              )}
            >
              {/* Preview */}
              <div className="relative"> {/* Relative container for the avatar preview and the checkmark badge */}
                ...
                {isSelected && ( // Only show the checkmark badge on the selected frame
                  <div className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-navy text-white"> {/* Small dark circle positioned at top-right of the preview */}
              ...
              {/* Label */}
              <p className="text-[9px] font-semibold text-black/80 leading-tight text-center"> {/* Very small centered label below the preview */}
      ...
      {/* Description */}
      <div className="rounded-lg bg-silver/20 border border-silver/30 px-3 py-2"> {/* Info box showing the description of the selected frame */}
        <p className="text-xs text-black/80"> {/* Small text for the description */}
